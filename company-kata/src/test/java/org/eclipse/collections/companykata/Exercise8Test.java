...
public class Exercise8Test extends CompanyDomainForKata
{
    ...
    public void totalOrderValuesByCityUsingPrimitiveValues()
    {
        ...
        ObjectDoubleMap<String> map = this.company.getCustomers().sumByDouble(Customer::getCity, Customer::getTotalOrderValue);
        Verify.assertSize(2, map);
        Assert.assertEquals(446.25, map.get("London"), 0.0);
        Assert.assertEquals(857.0, map.get("Liphook"), 0.0);
    }
    ...
    public void whoOrderedSaucers()
    {
        MutableList<Customer> customersWithSaucers = this.company.getCustomers().select(
                customer -> customer.getOrders().anySatisfy(
                        order -> order.getLineItems().anySatisfy(item -> "saucer".equals(item.getName()))));
    ...
    {
        MutableListMultimap<Double, Customer> multimap = this.company.getCustomers().groupBy(
                customer -> customer.getOrders().asLazy()
                        .flatCollect(Order::getLineItems)
                        .collectDouble(LineItem::getValue)
                        .max());
