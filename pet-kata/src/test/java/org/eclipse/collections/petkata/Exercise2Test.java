...
public class Exercise2Test extends PetDomainForKata
{
    ...
    {
        Predicate<Person> predicate = p -> p.hasPet(PetType.CAT); //replace null with a Predicate lambda which checks for PetType.CAT
    ...
    {
        Person result = this.people.detect(p -> "Mary".equals(p.getFirstName()) && "Smith".equals(p.getLastName()));
    ...
    public void getAllPetsOfAllPeople()
    {
        ...
        MutableSet<PetType> petTypes = Sets.mutable.withAll(this.people.flatCollect(function));
        Assert.assertEquals(
